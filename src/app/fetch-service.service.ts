...
import { collection, collectionData, Firestore, query, where, getDocs, getDoc, addDoc, runTransaction, doc, deleteDoc, Timestamp} from '@angular/fire/firestore';
...
export class FetchServiceService implements OnInit {
  ...
  async getExerciseSetDataForDay(userId: string, day: Date): Promise<ExerciseSet[]> {
    ...
    try {
        ...
        for (const dataDoc of dataSnapshot.docs) {
            ...
            const setsQuery = query(this.setsCollection, where('dataId', '==', dataDoc.id));
            const setsSnapshot = await getDocs(setsQuery);
            const sets: Set[] = setsSnapshot.docs.map((setDoc: any) => setDoc.data() as Set);
  ...
  async addSetstoExerciseOnDay(userId: string, day: Date, exerciseId: string, sets: Set[]): Promise<string[]> {
    ...
    return await runTransaction(this.firestore, async (transaction: any) => {
      ...
      if (!dataSnapshot.empty) {
        ...
        const newDataDoc = await addDoc(this.dataCollection, { userId, date: Timestamp.fromDate(day), exerciseId, totalVolume: 0 });
      ...
      transaction.update(doc(this.firestore, "data", dataDocId), { totalVolume: newTotalVolume });
      return setIds;
    });
  }

  // deletes an exercise and all its sets for a given day
  async deleteExerciseWithSetsForDay(userId: string, day: Date, exerciseId: string): Promise<void> {
    ...
    console.log(`Deleted all sets and data for exercise ${exerciseId} on ${day.toDateString()}`);
  }
  ...
  addExercisesFromJSON()
  {
    for(const exercise of exerciseData)
      {
        this.addExercise(exercise.name, exercise.muscleGroupWorked, exercise.description);
      }
  }
  ...
  async addThreeExerciseSets(userId: string, day: Date): Promise<void> {
    try {
      ...
      const exercises: { id: string; name: string; description: string; muscleGroupWorked: number; }[] = [];
  ...
  async addExerciseSetsForXDays(userId: string, startDate: Date, numberOfDays: number): Promise<void> {
