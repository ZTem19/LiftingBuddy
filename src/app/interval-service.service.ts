...
export class IntervalServiceService {
  ...
  async GetIntervalProgress(startDate: Date, numberOfDaysPerInterval: number, numberOfIntervals: number): Promise<Map<MuscleGroup, MuscleGroupProgress>> {
    ...
    let chestProgress: MuscleGroupProgress = {
      ...
      volumeLastInterval: 0,
      volumeMax: 0,
      volumeMin: 0,
    };
    let backProgress: MuscleGroupProgress = {
      volumeFirstInterval: 0,
      volumeLastInterval: 0,
      volumeMax: 0,
      volumeMin: 0,
    };
    let tricepProgress: MuscleGroupProgress = {
      volumeFirstInterval: 0,
      volumeLastInterval: 0,
      volumeMax: 0,
      volumeMin: 0,
    };
    let bicepProgress: MuscleGroupProgress = {
      volumeFirstInterval: 0,
      volumeLastInterval: 0,
      volumeMax: 0,
      volumeMin: 0,
    };
    let glutesQuadsProgress: MuscleGroupProgress = {
      volumeFirstInterval: 0,
      volumeLastInterval: 0,
      volumeMax: 0,
      volumeMin: 0,
    };
    let hamstringsCalvesProgress: MuscleGroupProgress = {
      volumeFirstInterval: 0,
      volumeLastInterval: 0,
      volumeMax: 0,
      volumeMin: 0,
    ...
    for (let i = 0; i < numberOfIntervals; i++) {
      ...
      let currentIntervalMap = await this.GetVolumeMuscleGroup(
        intervalStartDate,
        intervalEndDate
      );
      ...
      if (i == 0) {
        chestProgress.volumeFirstInterval =
          currentIntervalMap.get(MuscleGroup.Chest) ?? 0;
        backProgress.volumeFirstInterval =
          currentIntervalMap.get(MuscleGroup.Back) ?? 0;
        tricepProgress.volumeFirstInterval =
          currentIntervalMap.get(MuscleGroup.Triceps) ?? 0;
        bicepProgress.volumeFirstInterval =
          currentIntervalMap.get(MuscleGroup.Biceps) ?? 0;
        glutesQuadsProgress.volumeFirstInterval =
          currentIntervalMap.get(MuscleGroup.GlutesQuads) ?? 0;
        hamstringsCalvesProgress.volumeFirstInterval =
          currentIntervalMap.get(MuscleGroup.HamstringsCalves) ?? 0;

        chestProgress.volumeMin =
          currentIntervalMap.get(MuscleGroup.Chest) ?? 0;
        backProgress.volumeMin = currentIntervalMap.get(MuscleGroup.Back) ?? 0;
        tricepProgress.volumeMin =
          currentIntervalMap.get(MuscleGroup.Triceps) ?? 0;
        bicepProgress.volumeMin =
          currentIntervalMap.get(MuscleGroup.Biceps) ?? 0;
        glutesQuadsProgress.volumeMin =
          currentIntervalMap.get(MuscleGroup.GlutesQuads) ?? 0;
        hamstringsCalvesProgress.volumeMin =
          currentIntervalMap.get(MuscleGroup.HamstringsCalves) ?? 0;

        chestProgress.volumeMax =
          currentIntervalMap.get(MuscleGroup.Chest) ?? 0;
        backProgress.volumeMax = currentIntervalMap.get(MuscleGroup.Back) ?? 0;
        tricepProgress.volumeMax =
          currentIntervalMap.get(MuscleGroup.Triceps) ?? 0;
        bicepProgress.volumeMax =
          currentIntervalMap.get(MuscleGroup.Biceps) ?? 0;
        glutesQuadsProgress.volumeMax =
          currentIntervalMap.get(MuscleGroup.GlutesQuads) ?? 0;
        hamstringsCalvesProgress.volumeMax =
          currentIntervalMap.get(MuscleGroup.HamstringsCalves) ?? 0;
      }

      // set last interval volumes if last interval
      if (i == numberOfIntervals - 1) {
        chestProgress.volumeLastInterval =
          currentIntervalMap.get(MuscleGroup.Chest) ?? 0;
        backProgress.volumeLastInterval =
          currentIntervalMap.get(MuscleGroup.Back) ?? 0;
        tricepProgress.volumeLastInterval =
          currentIntervalMap.get(MuscleGroup.Triceps) ?? 0;
        bicepProgress.volumeLastInterval =
          currentIntervalMap.get(MuscleGroup.Biceps) ?? 0;
        glutesQuadsProgress.volumeLastInterval =
          currentIntervalMap.get(MuscleGroup.GlutesQuads) ?? 0;
        hamstringsCalvesProgress.volumeLastInterval =
          currentIntervalMap.get(MuscleGroup.HamstringsCalves) ?? 0;
      }

      // calculate min volume based on previous min
      chestProgress.volumeMin = Math.min(
        currentIntervalMap.get(MuscleGroup.Chest) ?? 0,
        chestProgress.volumeMin
      );
      backProgress.volumeMin = Math.min(
        currentIntervalMap.get(MuscleGroup.Back) ?? 0,
        backProgress.volumeMin
      );
      tricepProgress.volumeMin = Math.min(
        currentIntervalMap.get(MuscleGroup.Triceps) ?? 0,
        tricepProgress.volumeMin
      );
      bicepProgress.volumeMin = Math.min(
        currentIntervalMap.get(MuscleGroup.Biceps) ?? 0,
        bicepProgress.volumeMin
      );
      glutesQuadsProgress.volumeMin = Math.min(
        currentIntervalMap.get(MuscleGroup.GlutesQuads) ?? 0,
        glutesQuadsProgress.volumeMin
      );
      hamstringsCalvesProgress.volumeMin = Math.min(
        currentIntervalMap.get(MuscleGroup.HamstringsCalves) ?? 0,
        hamstringsCalvesProgress.volumeMin
      );

      // calculate max volume based on previous max
      chestProgress.volumeMax = Math.max(
        currentIntervalMap.get(MuscleGroup.Chest) ?? 0,
        chestProgress.volumeMax
      );
      backProgress.volumeMax = Math.max(
        currentIntervalMap.get(MuscleGroup.Back) ?? 0,
        backProgress.volumeMax
      );
      tricepProgress.volumeMax = Math.max(
        currentIntervalMap.get(MuscleGroup.Triceps) ?? 0,
        tricepProgress.volumeMax
      );
      bicepProgress.volumeMax = Math.max(
        currentIntervalMap.get(MuscleGroup.Biceps) ?? 0,
        bicepProgress.volumeMax
      );
      glutesQuadsProgress.volumeMax = Math.max(
        currentIntervalMap.get(MuscleGroup.GlutesQuads) ?? 0,
        glutesQuadsProgress.volumeMax
      );
      hamstringsCalvesProgress.volumeMax = Math.max(
        currentIntervalMap.get(MuscleGroup.HamstringsCalves) ?? 0,
        hamstringsCalvesProgress.volumeMax
      );
